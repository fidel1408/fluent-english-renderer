...
const totalWidth =
wordWidths.reduce((a, b) => a + b, 0) + wordSpacing * Math.max(0, words.length - 1);
...
```
  return {
    candidate,
    score: distanceFromHead - tailDistance - movement * 0.2,
  };
})
...
if (side === "left") {
  ...
  bounds.x = clamp(
    bounds.x,
    Math.min(WIDTH - bubbleWidth - margin, WIDTH / 2 + 22),
...
const minBubbleWidth = 230;
const maxBubbleWidth = 505;
const requestedMaxWidth =
typeof bubble.width === "number" ? clamp(bubble.width, minBubbleWidth, maxBubbleWidth) : maxBubbleWidth;

const minBubbleHeight =
typeof bubble.min_height === "number" ? Math.max(72, bubble.min_height) : 76;
...
const initialTextMaxWidth = requestedMaxWidth - paddingX * 2 - numberColumnWidth;
const lines = wrapWords(initialTextMaxWidth);
...
const naturalBubbleWidth = Math.ceil(widestLine + paddingX * 2 + numberColumnWidth);

const bubbleWidth = clamp(
Math.min(requestedMaxWidth, naturalBubbleWidth),
minBubbleWidth,
requestedMaxWidth
);

const finalTextMaxWidth = bubbleWidth - paddingX * 2 - numberColumnWidth;
const finalLines = wrapWords(finalTextMaxWidth);

const contentHeight = finalLines.length * rowHeight - rowGap;
...
finalLines.forEach((lineIndices, lineIndex) => {
...
```
let wx = textStartX + (finalTextMaxWidth - lineWidth) / 2;
...
lineIndices.forEach((idx) => {
  ...
  if (isHighlight) {
    wordParts.push(
      `<rect x="${wx - 5}" y="${baseY - wordFontSize - 3}" width="${wordWidth + 10}" height="${wordFontSize + 8}" fill="rgba(255,224,36,0.94)" stroke="rgba(30,30,30,0.14)" stroke-width="1" rx="7"/>`
    );
  }
...
return {
bounds,
svg: `       <g filter="url(#bubbleShadow)">         <path d="${tailPath}" fill="#ffffff" stroke="#111111" stroke-width="2.6" stroke-linejoin="round"/>         <rect x="${bounds.x}" y="${bounds.y}" width="${bounds.w}" height="${bounds.h}" rx="${radius}" ry="${radius}" fill="#ffffff" stroke="#111111" stroke-width="2.8"/>         <rect x="${bounds.x + 5}" y="${bounds.y + 5}" width="${bounds.w - 10}" height="${bounds.h - 10}" rx="${radius - 6}" ry="${radius - 6}" fill="none" stroke="rgba(255,255,255,0.82)" stroke-width="1.4"/>       </g>       <circle cx="${bounds.x + paddingX + 16}" cy="${bounds.y + bounds.h / 2}" r="18" fill="url(#badgeBlue)" stroke="#0B3F91" stroke-width="2"/>       <circle cx="${bounds.x + paddingX + 10}" cy="${bounds.y + bounds.h / 2 - 7}" r="5" fill="rgba(255,255,255,0.48)"/>       <text x="${bounds.x + paddingX + 16}" y="${bounds.y + bounds.h / 2 + 6}" font-family="Trebuchet MS, Arial, sans-serif" font-size="17" font-weight="800" fill="white" text-anchor="middle">${bubble.number}</text>
      ${wordParts.join("\n")}
    `,
};
}
...
const busyTopArea: Rect = { x: 0, y: 0, w: WIDTH, h: 310 };
const topHasBubbleClutter = bubbleBounds.some((bubble) => bubble.y < 310);
...
```
  const bubblePenalty = bubbleBounds.reduce((penalty, bubble) => {
    ...
    const proximityPenalty = Math.max(0, 380 - distance) * 2.2;

    return penalty + overlap * 18 + proximityPenalty;
  }, 0);

  const titlePenalty = rectOverlapArea(logoRect, titleSafeArea) * 22;

  const busyTopPenalty = position.startsWith("top")
    ? 950 + rectOverlapArea(logoRect, busyTopArea) * 2
    : 0;

  const roleplayTopPenalty =
    topHasBubbleClutter && position.startsWith("top") ? 1700 : 0;

  const bottomPreference = position.startsWith("bottom") ? 360 : 0;
...
return scored[0]?.position ?? "bottom-right";
}
...
const textFill =
logo.variant === "white" ? "white" : logo.variant === "black" ? "#111111" : "#1565C0";
...
return `     <g filter="url(#logoBadgeShadow)">       <text x="${rect.x}" y="${rect.y + 62}" font-family="Georgia, serif" font-size="38" font-weight="bold" fill="${textFill}" stroke="${logo.variant === "white" ? "rgba(0,0,0,0.48)" : "rgba(255,255,255,0.58)"}" stroke-width="2.4" paint-order="stroke">Fluent English</text>     </g>
  `;
}

async function buildOverlaySvg(body: SlideRequest, bgSharp: Sharp): Promise<string> {
const rawTitle = body.main_title ?? "";
const titleWords =
body.main_title_words && body.main_title_words.length
? body.main_title_words
: rawTitle
? rawTitle.split(/\s+/).filter(Boolean)
: [];

const titleIPA =
body.main_title_ipa && body.main_title_ipa.length
? body.main_title_ipa
: titleWords.map(() => "");
...
const bubbles = (isSceneSlide ? (body.speech_bubbles ?? []).slice(0, 2) : body.speech_bubbles ?? []).map(
(b) => buildBubbleSvg(b)
);
...
body.show_title === true ||
(body.show_title !== false && typeof body.slide_number === "number" && body.slide_number === 1);

return `<?xml version="1.0" encoding="UTF-8"?> <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${WIDTH}" height="${HEIGHT}"> <defs> <filter id="titleShadow" x="-10%" y="-10%" width="120%" height="130%"> <feDropShadow dx="2" dy="2" stdDeviation="3" flood-color="rgba(0,0,0,0.48)"/> </filter> <filter id="bubbleShadow" x="-8%" y="-12%" width="124%" height="138%"> <feDropShadow dx="0" dy="7" stdDeviation="7" flood-color="rgba(0,0,0,0.24)"/> </filter> <filter id="logoBadgeShadow" x="-8%" y="-14%" width="124%" height="138%"> <feDropShadow dx="0" dy="8" stdDeviation="10" flood-color="rgba(0,0,0,0.22)"/> </filter> <linearGradient id="badgeBlue" x1="0" y1="0" x2="0" y2="1"> <stop offset="0%" stop-color="#2B8CFF"/> <stop offset="52%" stop-color="#1565C0"/> <stop offset="100%" stop-color="#0B3F91"/> </linearGradient> <linearGradient id="brightWash" x1="0" y1="0" x2="1" y2="1"> <stop offset="0%" stop-color="rgba(255,255,255,0.14)"/> <stop offset="45%" stop-color="rgba(255,255,255,0.03)"/> <stop offset="100%" stop-color="rgba(255,235,160,0.10)"/> </linearGradient> </defs>
